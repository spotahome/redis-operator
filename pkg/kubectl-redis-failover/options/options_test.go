...
import (
	...
	fakeoptions "github.com/spotahome/redis-operator/pkg/kubectl-redis-failover/options/fake"
)

func TestExample(t *testing.T) {
	tf, o := fakeoptions.NewFakeRedisFailoverOptions()
	defer tf.Cleanup()
	example := `
  # do something
  %[1]s foo
`
	assert.Equal(t, "  # do something\n  kubectl redis failover foo", o.Example(example))
}
