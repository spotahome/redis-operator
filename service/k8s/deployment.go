...
func (d *DeploymentService) GetDeployment(namespace, name string) (*appsv1.Deployment, error) {
	deployment, err := d.kubeClient.AppsV1().Deployments(namespace).Get(context.Background(), name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (d *DeploymentService) GetDeploymentPods(namespace, name string) (*corev1.PodList, error) {
	...
	return d.kubeClient.CoreV1().Pods(namespace).List(context.Background(), metav1.ListOptions{LabelSelector: selector})
}
...
func (d *DeploymentService) CreateDeployment(namespace string, deployment *appsv1.Deployment) error {
	_, err := d.kubeClient.AppsV1().Deployments(namespace).Create(context.Background(), deployment, metav1.CreateOptions{})
	if err != nil {
		return err
	}
...
func (d *DeploymentService) UpdateDeployment(namespace string, deployment *appsv1.Deployment) error {
	_, err := d.kubeClient.AppsV1().Deployments(namespace).Update(context.Background(), deployment, metav1.UpdateOptions{})
	if err != nil {
		return err
	}
...
func (d *DeploymentService) DeleteDeployment(namespace, name string) error {
	...
	return d.kubeClient.AppsV1().Deployments(namespace).Delete(context.Background(), name, metav1.DeleteOptions{PropagationPolicy: &propagation})
}
...
func (d *DeploymentService) ListDeployments(namespace string) (*appsv1.DeploymentList, error) {
	return d.kubeClient.AppsV1().Deployments(namespace).List(context.Background(), metav1.ListOptions{})
}
