...
func (s *ServiceService) GetService(namespace string, name string) (*corev1.Service, error) {
	service, err := s.kubeClient.CoreV1().Services(namespace).Get(context.Background(), name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	return service, err
...
func (s *ServiceService) UpdateService(namespace string, service *corev1.Service) error {
	_, err := s.kubeClient.CoreV1().Services(namespace).Update(context.Background(), service, metav1.UpdateOptions{})
	if err != nil {
		return err
	}
...
func (s *ServiceService) DeleteService(namespace string, name string) error {
	...
	return s.kubeClient.CoreV1().Services(namespace).Delete(context.Background(), name, metav1.DeleteOptions{PropagationPolicy: &propagation})
}

func (s *ServiceService) ListServices(namespace string) (*corev1.ServiceList, error) {
	return s.kubeClient.CoreV1().Services(namespace).List(context.Background(), metav1.ListOptions{})
}
